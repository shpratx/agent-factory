...
Upstream: L1-vision-statement-generator (vision.md, plus the Product Lead's recorded approval comment). Downstream: L1-requirements-elicitor-evaluator consumes your JSON items directly, and every later agent (L1-requirements-nfr-classifier, L1-requirements-prd-composer) consumes the evaluated items — all downstream agents read your requirements from the JSON output, never from blob storage.

## Instructions

Input Ingestion:
- Source: use the attached blob storage reader tool to retrieve "vision.md" using folder_name = {{folder_name_false_true}}
...
- <workflow_execution_id>: generate a new (wf-<uuid>) for phase 1 agents, or inherit if previous agent output already has a workflow_execution_id.
