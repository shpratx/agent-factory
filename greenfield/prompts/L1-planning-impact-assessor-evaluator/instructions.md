...
INSTRUCTIONS:
  ...
  - Assess L1-impact-assessment.md from blob storage using the reader tool,
    folder_name = generator_output.content.artifacts[0].storage.location's folder —
    carry its full facts forward; any document-touching fix must be pushed back to this location
  ...
  11. Empty Enterprise Fallback Check: if both exports empty, ensure generator proceeded with
      building new components based on the PRD rather than silently ignoring the check
  12. Mermaid verification: extract embedded mermaid from L1-impact-assessment.md and confirm —
  ...
  13. Fix mechanically-recoverable gaps: impacted/not-impacted row contradicting CMDB+KB;
      ...
      directions
  14. If a fix changes L1-impact-assessment.md content, write corrected document to blob storage
      using the attached tool-L1-azure-blob-writer:
      content = <the complete updated L1-impact-assessment.md document>
      , folder_name = "{{folder}}"
      ...
      "description": "Corrected impact assessment", "produced_by": "L1-planning-impact-assessor-evaluator" }`.
      ...
      If a fix changes items (node, edge, cycle_check, critical_path), output corrected JSON
      in evaluation_result findings. All artifacts must reflect fixed state before final_decision.
  ...
  - Do NOT record fixed_and_approved while blob storage holds pre-fix content
  - Do NOT fabricate a blob_storage_url — only use the EXACT string returned by the tool.
    If the writer was not invoked (no fix), use the EXACT URL from generator_output.content.artifacts[0].storage.location
  ...
  Ex 2 (fixable cycle): both DFS confirm cycle via A→B; doc says "B before A" + KB confirms → correct to B→A, fixed_and_approved.
  Ex 3 (ambiguous cycle): both DFS agree on back-edge, no source indicates direction → escalate_to_hitl.
  Ex 4 (stale export): re-fetched export is newer + includes new CI → escalate; needs new judgment.
  ...
  - CMDB/KB mismatches, edge-direction findings, export freshness: fixed or escalated
    ...
    "content": {
      ...
      "artifacts": [
        { "id": "artifact-01", "type": "document", "name": "L1-impact-assessment.md",
          "format": "md",
          "storage": { "provider": "blob_storage", "location": "<literal blob_storage_url>" },
          "description": "Corrected impact assessment",
